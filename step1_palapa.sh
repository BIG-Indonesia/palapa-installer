...
installer_dir=$(pwd)
echo "============================================================================"
echo "Installer PALAPA v07 (Branch AGR)"
echo "============================================================================"
...
rpm -iUvh https://download.postgresql.org/pub/repos/yum/9.5/redhat/rhel-7.3-x86_64/pgdg-centos95-9.5-3.noarch.rpm
...
yum -y install python-devel python-virtualenv git-core java-1.8.0-openjdk python-pip postgis2_95 postgis2_95-utils postgis2_95-client libpg-devel postgresql95-devel postgresql95-server xml-commons git subversion mercurial libxslt libxslt-devel libxml2 libxml2-devel gcc gcc-c++ make java-1.8.0-openjdk-devel tomcat tomcat-webapps tomcat-admin-webapps xalan-j2 unzip policycoreutils-python mod_wsgi httpd wget mc nano firewalld libtidy libtidy-devel gdal gdal-python gdal-devel uwsgi uwsgi-plugin-python
...
su postgres -c "pg_restore -d palapa ${installer_dir}/paket/palapa_init_v6.backup" > /dev/null 2>&1
...
gspalapa_repo="-b agrisoft-main --single-branch https://github.com/BIG-Indonesia/palapa.git"
gspalapa_frontend_repo="-b agrisoft-main --single-branch https://github.com/BIG-Indonesia/palapa-frontend.git"
gspalapa_api_repo="-b agrisoft-main --single-branch https://github.com/BIG-Indonesia/palapa-api.git"
#pycsw_tarball "${installer_dir}/paket/pycsw.tar.gz"
# tar -xzf ${gspalapa_tarball} -C /var/www/html/
cd /var/www/html/
git clone ${gspalapa_repo} /var/www/html/gspalapa/
# tar -xzf ${gspalapa_frontend_tarball} -C /var/www/html/
git clone ${gspalapa_frontend_repo} /var/www/html/palapa/
# mkdir -p /opt/gspalapa-api/
mkdir -p /opt/pycsw-2.0/
git clone ${gspalapa_api_repo} /opt/gspalapa-api/
...
echo "============================================================================"
echo "Menseting GSPapala API daemon"
cp /opt/gspalapa-api/gs-api.service /etc/systemd/system/
systemctl daemon-reload
systemctl enable gs-api
...
sed -ie "s/REPGSMANAGERDOM/${domain_publik}/g" /opt/gspalapa-api/cfg.py

echo "============================================================================"
echo "Mengkopikan konfigurasi HTTPd"
pycsw_conf="${installer_dir}/paket/pycsw.conf"
gspalapa_conf="${installer_dir}/paket/gspalapa.conf"
cp ${pycsw_conf} /etc/httpd/conf.d/pycsw.conf
cp ${gspalapa_conf} /etc/httpd/conf.d/gspalapa.conf

echo "============================================================================"
echo "Menseting HTTPd config"
cd /etc/httpd/conf/
sed -ie "s/\/var\/www\/html/\/var\/www\/html\/palapa/g" /etc/httpd/conf/httpd.conf

echo "============================================================================"
echo "Menseting aplikasi backend GSPalapa"
cd /var/www/html/gspalapa/js/
sed -ie "s/REPGSMANAGERDOM/${domain_publik}/g" /var/www/html/gspalapa/js/cfg.js

echo "============================================================================"
echo "Menseting aplikasi frontend GSPalapa"
cd /var/www/html/palapa/js/
sed -ie "s/REPGSMANAGERDOM/${domain_publik}/g" /var/www/html/palapa/js/main.js

echo "============================================================================"
echo "Menjalankan daemon HTTPd dan GSPalapa API"
systemctl start httpd
systemctl start gs-api
